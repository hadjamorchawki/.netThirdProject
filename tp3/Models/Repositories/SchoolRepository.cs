namespace tp3.Models.Repositories
{
    public class SchoolRepository : ISchoolRepository
    {
        ...
        public List<School> Search(string term)
        {
            if (string.IsNullOrEmpty(term))
            {
                return new List<School>(); // Return an empty list if the search term is null or empty
            }

            return context.Schools
                .Where(s => s.SchoolName.Contains(term) )
                .ToList();
        }
    }
